#include "patchs/lenovo/avb_key_swap.h"
#include "arm64_inst/utils.h"
#include <string.h>

/* ARB variant: overwrite the embedded platform AvbRSAPublicKey with the
 * AOSP test key so test-key-signed vbmeta images verify.  The anchor is the
 * key header plus the first 32 modulus bytes; the replacement is the full
 * published testkey blob (header + n0inv + modulus + rr).
 */

/* AvbRSAPublicKey header (8 bytes) + first 32 modulus bytes of the
 * stock vbmeta key. memcmp anchor for in-place swap. */
static const uint8_t STOCK_AVB_ANCHOR[40] = {
    0x00, 0x00, 0x10, 0x00, 0xbc, 0x29, 0xf0, 0xad,
    0x99, 0x83, 0x5c, 0x83, 0xd5, 0x0e, 0x5d, 0xbc,
    0x55, 0x73, 0xa1, 0x33, 0xc1, 0x70, 0x7c, 0x62,
    0xee, 0x53, 0x5e, 0x1f, 0xa1, 0x39, 0xbd, 0x46,
    0x96, 0xb5, 0x50, 0x53, 0xc8, 0x80, 0x08, 0x86,
};

/* Full AvbRSAPublicKey blob for the AOSP testkey_rsa4096 — 8B header
 * + 4B n0inv + 512B modulus + 512B rr. Published under Apache 2.0 in
 * external/avb/test/data/testkey_rsa4096.pem. */
static const uint8_t AOSP_AVB_BLOB[1032] = {
    ,
};

#define AVB_ANCHOR_LEN 40
#define AVB_BLOB_LEN   1032

int32_t patch_swap_avb_key_arb(char* buffer, int32_t size) {
    if (size < AVB_BLOB_LEN) {
        printf("avb key swap: buffer too small, skipping\n");
        return 0;
    }

    int32_t patched = 0;
    int32_t limit = size - AVB_BLOB_LEN;
    for (int32_t i = 0; i <= limit; ++i) {
        if (memcmp(buffer + i, STOCK_AVB_ANCHOR, AVB_ANCHOR_LEN) != 0)
            continue;

        printf("avb key swap: stock AvbRSAPublicKey @ 0x%X -> AOSP testkey"
               " (%d bytes)\n", i, AVB_BLOB_LEN);
        memcpy(buffer + i, AOSP_AVB_BLOB, AVB_BLOB_LEN);
        patched++;
        i += AVB_BLOB_LEN - 1;
    }

    if (patched == 0)
        printf("avb key swap: stock key not located, skipping\n");
    else if (patched > 1)
        printf("avb key swap: warning, %d sites swapped (expected 1)\n",
               patched);
    return patched;
}
